...
async function generateBaseOntology() {
  console.log('🔄 Generating base ontology structure...');
  
  const ontologyContent = `@prefix : <http://www.matchcare.com/ontology#> .
...
`;
  ...
  // Save base ontology
  const outputPath = path.join(__dirname, '../data/ontology/base_ontology.ttl');
  await fs.writeFile(outputPath, ontologyContent);
  
  console.log(`✅ Base ontology created: ${outputPath}`);
  return outputPath;
}

// Run generator
generateBaseOntology().catch(console.error);